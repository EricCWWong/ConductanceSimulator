...
'''
    The following section contains functions to plot the conductance plot when
    given energy of each energy level.
'''
...
def single_transmission(threshold_energy, x):
    ...
    t: float
        ...
        threshold frequency, then the transmission will be 1. Otherwise, there
...
def total_transmission(energy_lvls, x):
    '''
        ...
        when given threshold energies of all the concerned levels of the system.

    Parameters
    ----------
    energy_lvls: float array
        This is an array of energy of each energy levels of a particular system. 
        First item (index [0]) in the array corresponds to the ground state of the system.
    x : float
        This is the x-axis parameter of energy. For a given energy x, 
        if x > threshold energy of a certain level, then we will have a 2e^2/h 
        jump in conductance.
    ...
    # The conductance of a certain energy x can be calculated as the sum of allowed
    # transmission:
...
def conductance_plot(energies, offset):
    '''
        This function plots the conductance curves when energy levels of the system
        in various set up is provided. It allows clear comparision between the 
        conductance plot for different experimental setup. I plot is shown when the
        function is completed.

    Parameters
    ----------
    energies: float array
        This is a 2D array. For example, energies[i][j] corresponds to the energy of
        the ith level in the jth experiment.
    offset : float
        This is the distance between each conductance curve to allow each curve to be
        seen clearly.
